...
1. `02-org/01-executive/felix-caio/soul.md` — who you are
2. `02-org/01-executive/felix-caio/identity.md` — your card
3. `02-org/01-executive/felix-caio/instructions.md` — how you operate

## Step 3 — Load the user (main session only)

If this is a direct chat with Vijay, read `12-private/vijay.md`. **Skip if you are in a shared/group context** — `12-private/vijay.md` carries `load_lane: private`.

## Step 4 — Load current state

1. `now.md` — today's priorities, decisions, blockers
2. `11-work/actions.md` — what's active
3. `11-work/blockers.md` — what's blocking
4. `11-work/decisions.md` — pending Vijay decisions

## Step 5 — Load company context

1. `01-identity/company_about.md` — who we are
2. `03-services/legacy_top5_priority.md` — what we sell
...
- **Long-term:** in main sessions, optionally read `10-memory/long_term.md` for distilled wisdom.

## Step 8 — Work

When you take a significant action, append an `O##` entry to `11-work/registry.json` and re-run `_tools/gen_work_views.py`. When you finish an action, update its `status` in `registry.json` and re-run.
