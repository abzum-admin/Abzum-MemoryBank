...
**Existing UCs** are sourced from [`work/registry.json`](../work/registry.json) (UC-01 through UC-20 are already defined). **New UCs** (UC-21+) added 2026-05-10 to capture cases the master plan didn't list separately.
...
When the request doesn't match any known UC, Triage routes to BA for full discovery, and a new UC row is added to `work/registry.json` after the BA call.
...
- [`company/use_cases.md`](../company/use_cases.md) — strategic use-case priority (top 5)
- [`work/registry.json`](../work/registry.json) — UC namespace
