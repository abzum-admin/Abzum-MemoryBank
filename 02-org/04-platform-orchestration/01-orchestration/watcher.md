...
## Function
The Watcher is a sidecar process that tails the Orchestrator's tool-call stream looking for trouble: stuck loops (hash-fingerprint tool-call repeats), stalls (no progress over N turns), anomalous tool patterns, and budget breaches. It posts structured alerts back to the Orchestrator and escalates per the [`06-infrastructure/03-services/agent_watcher.md`](../../../06-infrastructure/03-services/agent_watcher.md) ladder.
...
- Budget gauge: reads ClickHouse cost events (see `05-process/memory_protocol.md`)
