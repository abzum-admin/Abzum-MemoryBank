...
Full runbook: [setup_app.md](setup_app.md)
...
>
> **Managed by:** `abzum-setup-app` boot orchestrator (previously `hermes-felix.service` systemd unit). On VPS reboot the setup app automatically runs `doppler run -- docker compose up -d` for this installation, refreshing Doppler secrets. To install, update, or remove via the UI, see [setup_app.md](setup_app.md).
